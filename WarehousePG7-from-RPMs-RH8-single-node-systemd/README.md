# WarehousePG 7 — single node, RPMs, RHEL8, **systemd**

Same single-node WarehousePG 7 image as
`../WarehousePG7-from-RPMs-RH9-single-node-systemd`, but built on
`rockylinux:8` instead of `rockylinux:9`. This is the first RHEL8-based
lab in this repo. Like its RH9 sibling, the container boots
...
- `warehousepg.service` — oneshot unit: `ExecStartPre` runs the prep
  ...
  runs `gpstop`. `gpinitsystem` is tolerated with `|| true` since it
  exits 1 on a benign trust-auth `pg_hba.conf` WARN even on full success.
...
make run
make status
make psql        # connect via host port 6441
