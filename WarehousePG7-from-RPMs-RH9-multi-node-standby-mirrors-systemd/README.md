...
- Host ports: coordinator `6443:5432`, standby `6444:5432` (the plain
  multi-node systemd lab uses 6441/6442 — these are separate labs and can
  run side by side).
...
make run
make status
make psql-coordinator   # connect via host port 6443
make psql-standby       # connect via host port 6444
