...
# WarehousePG-bundled Python needs psycopg2 + psutil for gpstart/gpinitsystem
# (only actually used on the coordinator, but harmless everywhere).
# python2-psutil already comes from the RPM list below, so this loop is a
# no-op for it; psycopg2 has no EPEL7 RPM, so it always needs the pip
# fallback -- install the prebuilt-wheel "-binary" form either way.
...
# export MASTER_DATA_DIRECTORY on coordinator/standby so gpstate/gpstart/gpstop
...
if [ "${HOSTNAME}" == "coordinator" -o "${HOSTNAME}" == "standby" ]; then
    if ! grep -q "export MASTER_DATA_DIRECTORY=" /home/${WHPG_USER}/.bashrc 2>/dev/null; then
        echo "export MASTER_DATA_DIRECTORY=/whpgdata/master/whpgmne-1" >> /home/${WHPG_USER}/.bashrc
    fi
fi
