# WarehousePG 6 — multi-node with standby master + mirrors, RPMs, RHEL9, **systemd**
...
adapted from `../WarehousePG7-from-RPMs-RH9-multi-node-standby-mirrors-systemd`.
...
- WarehousePG 6 tooling (`gpinitsystem`/`gpexpand`) is Python 2 based.
  RHEL9/Rocky9 dropped python2 entirely (no OS package, no module
  stream); EDB ships its own interpreter as `edb-python27` instead,
  pulled in automatically as a dependency of `warehouse-pg-6` — see the
  comment block in `Dockerfile` above the `alternatives --install` lines.
...
- Host ports: coordinator `6457:5432`, standby `6458:5432` (separate from
...
make run
make status
make psql-coordinator   # connect via host port 6457
make psql-standby       # connect via host port 6458
