# WarehousePG 7 — multi-node with standby coordinator + mirrors, RPMs, RHEL8, **systemd**

Same 5-container multi-node WarehousePG 7 cluster as
`../WarehousePG7-from-RPMs-RH9-multi-node-standby-mirrors-systemd`
(coordinator, standby, seg1, seg2, gpfdist — with a standby coordinator
and mirror segments enabled), but built on `rockylinux:8` instead of
`rockylinux:9`. Every container boots **systemd as PID 1** and manages
...
- Host ports: coordinator `6452:5432`, standby `6453:5432` (the RH9
  variant of this lab uses 6448/6450 — these are separate labs and can
  run side by side).
...
make run
make status
make psql-coordinator   # connect via host port 6452
make psql-standby       # connect via host port 6453
