...
# across all 5 hosts).
#
# - coordinator: first boot -> gpinitsystem; later boots -> gpstart
...
#   their segment postgres instances up remotely via ssh from the
#   coordinator
...
case "${HOSTNAME}" in
  coordinator)
    if [ ! -f ${COORDINATOR_DATA_DIR}/whpgmne-1/postgresql.conf ]; then
        ...
        ( source ${WHPG_HOME}/greenplum_path.sh && gpinitsystem -c /home/${WHPG_USER}/whpginitsystem_multinode -m 100 -a ) || true
        ...
        ( source ${WHPG_HOME}/greenplum_path.sh && gpstart -a -d ${COORDINATOR_DATA_DIR}/whpgmne-1/ ) || true
